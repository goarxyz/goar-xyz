...
class VibeApp(App):  # noqa: PLR0904
    ...
    async def _maybe_show_feedback_bar(self) -> None:
        """Feedback surveys are intentionally absent in Alpine Private Vibe."""
        return None
...
"""
    ...
    async def _refresh_account(self) -> None:
        """Keep local UI state current without an account-service request."""
        self._refresh_command_registry()
        self._refresh_banner()

    async def _refresh_identity(self) -> None:
        """Identity reporting is intentionally absent in Alpine Private Vibe."""
        self._refresh_banner()
    ...
    def _schedule_update_notification(self) -> None:
        """Automatic update checks are intentionally absent in Alpine Private Vibe."""
        return None
    ...
    def _handle_exception(self, error: Exception) -> None:
        """Use Textual's local exception handling without crash reporting."""
        return super()._handle_exception(error)

    def on_worker_state_changed(self, event: Worker.StateChanged) -> None:
        """Worker failures remain local to the terminal process."""
        del event
...
def run_textual_ui(
    ...
    async def run() -> SessionExitSummary | None:
        ...
        # Privacy fork: no registry checks, update cache, or promotional service.
