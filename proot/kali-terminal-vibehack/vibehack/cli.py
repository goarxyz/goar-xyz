...
def default(
    ...
    sandbox: bool = typer.Option(False, "--sandbox", help="Use the GOAR app-private Kali PRoot execution guest"),
    ...
    """
    Interactive GOAR terminal REPL backed by VibeHack and a Kali PRoot guest.

    Run with no arguments to open the persistent AI terminal operator.
    Type natural-language tasks or use the full local terminal workflow.
    The Android launcher already enables the app-private PRoot execution guest.
...
def start(
    ...
    """Start a quick authorized assessment session in the PRoot guest."""
    if model:
        os.environ["VH_MODEL"] = model
        cfg.MODEL = model
