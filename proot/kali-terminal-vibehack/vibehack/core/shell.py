...
class PersistentSession:
    ...
    async def start(self):
        """Start one stateful shell in the already-contained GOAR PRoot guest."""
        from vibehack.core.sandbox import CONTAINER_NAME, start_sandbox

        start_sandbox()
        self.container_name = CONTAINER_NAME
        workspace = os.environ.get("GOAR_WORKSPACE", "/data/workspace")
        guest_path = "/usr/local/bin:/usr/local/sbin:/usr/sbin:/usr/bin:/sbin:/bin"
        guest_env = os.environ.copy()
        guest_env.update(
            {
                "PATH": guest_path,
                "TERM": guest_env.get("TERM", "xterm-256color"),
                "COLORTERM": guest_env.get("COLORTERM", "truecolor"),
                "GOAR_PROOT_GUEST": "1",
            }
        )
        shell = guest_env.get("SHELL", "/usr/bin/bash")
        ...
        try:
            ...
            print(f"DEBUG: Failed to start persistent GOAR PRoot session: {e}")
            self.process = None
    ...
    async def execute(
        ...
        if not self.process:
            return ShellResult(
                "",
                "Error: Persistent GOAR PRoot session could not be started. Check the terminal runtime log.",
                1,
                False,
            )
...
async def execute_shell(
    ...
    # The Android launcher creates the Kali PRoot guest before this process.
    if not check_docker():
        return ShellResult(
            "",
            "CRITICAL ERROR: GOAR PRoot runtime is not active. Launch through goar-terminal.",
            1,
            False,
        )
    ...
    if not is_container_running():
        ...
        try:
            ...
            return ShellResult(
                "", f"CRITICAL ERROR: Failed to prepare GOAR PRoot runtime: {str(e)}", 1, False
            )
...
async def _execute_stateless(
    ...
    return ShellResult(
        "",
        "CRITICAL ERROR: Host execution is permanently disabled. All commands MUST run inside the GOAR PRoot guest.",
        1,
        False,
    )
