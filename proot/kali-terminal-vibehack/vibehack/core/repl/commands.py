...
SLASH_COMMANDS = {
    ...
    "/goar":      "Control durable GOAR state (/goar status | plan | checkpoint | trust | loop | config | events)",
...
def _handle_goar_control(repl, arg: str):
    """Expose the durable GOAR terminal control plane inside the live REPL."""
    import subprocess
    import sys
    command = [sys.executable, "/opt/goar-terminal/goarctl.py"]
    if arg:
        command.extend(arg.split())
    else:
        command.append("status")
    env = os.environ.copy()
    env["GOAR_SESSION_ID"] = repl.session_id
    env.setdefault("GOAR_CONTROL_ROOT", "/data/goar/control")
    env.setdefault("GOAR_WORKSPACE", "/data/workspace")
    try:
        result = subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, timeout=20, env=env)
        text = (result.stdout or "").strip()
        if result.returncode == 0:
            log_to_pane(repl, "history", "[ GOAR CONTROL ]\\n" + (text or "OK"))
        else:
            log_to_pane(repl, "logs", "GOAR CONTROL ERROR: " + (text or f"exit {result.returncode}"))
    except Exception as error:
        log_to_pane(repl, "logs", "GOAR CONTROL ERROR: " + str(error))
