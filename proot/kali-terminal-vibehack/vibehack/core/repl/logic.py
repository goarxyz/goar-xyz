...
async def _execute_proposed_command(repl, response: AgentResponse):
    ...
    goar_core = getattr(repl, "_goar_core", None)
    if goar_core is not None:
        goar_core.record_tool("shell", {"command": cmd})
        goar_core.hooks.dispatch("pre_tool", {"name": "shell", "command": cmd})
    ...
    if goar_core is not None:
        goar_core.record_tool("shell", {"command": cmd}, result=f"exit={result.exit_code};stdout={result.stdout};stderr={result.stderr or ''}")
        goar_core.hooks.dispatch("post_tool", {"name": "shell", "exit_code": result.exit_code})
